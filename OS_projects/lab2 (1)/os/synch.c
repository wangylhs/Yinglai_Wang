...
int CondInit(Cond *c, lock_t lock) {
...
cond_t CondCreate(lock_t lock) {
  ...
  if(CondInit(&conds[c], lock) != SYNC_SUCCESS) return SYNC_FAIL;
  return c;
}
...
int CondWait(Cond *condition) {
  ...
  dbprintf ('s', "CondHandleWait: Proc %d waiting on condition %d, lock=%d.\n", GetCurrentPid(), (int)(condition-conds), condition->lock);

///////////////////////// Checking if this process owns the lock 
  if (locks[condition->lock].pid != GetCurrentPid()){ //lock acquired
	dbprintf('s', "Condwait: Proc %d does not own cond %d\n", GetCurrentPid(), (int)(condition-conds));
	RestoreIntrs (intrval);
	return SYNC_FAIL;
  }
//////////////////////////
    ...
    if (AQueueInsertLast (&condition->waiting, l) != QUEUE_SUCCESS) {
      printf("FATAL ERROR: could not insert new link into the condition variable  waiting queue in CondWait!\n");
      exitsim();
    }
    LockHandleRelease(condition->lock);
    ProcessSleep();
  }
  RestoreIntrs (intrval);
  LockHandleAcquire(condition->lock);
  return SYNC_SUCCESS;
}
...
int CondSignal (Cond *condition) {
  ...
  intrs = DisableIntrs ();
  dbprintf ('s', "CondSignal: Process %d Signaling the condition variable %d, lock=%d.\n", GetCurrentPid(), (int)(condition-conds), condition->lock);
  


  if  (locks[condition->lock].pid !=GetCurrentPid()){ 
	//the lock is not grabbed properlly
	printf("CondSignal: Process %d did not grab the lock for condition variable %d\n", GetCurrentPid(), (int)(condition-conds));
  	RestoreIntrs (intrs);
	return SYNC_FAIL;
  }
  else{
      if (!AQueueEmpty(&condition->waiting)) { // there is a process to waiting for the wake up
      l = AQueueFirst(&condition->waiting);
      pcb = (PCB *)AQueueObject(l);
      if (AQueueRemove(&l) != QUEUE_SUCCESS) { 
        printf("FATAL ERROR: could not remove link from the condition variable queue in CondSignal!\n");
        exitsim();
      }
...
int CondBroadcast (Cond *condition) {
  Link *l;
  int	intrs;
  PCB *pcb;
  ...
  if (locks[condition->lock].pid !=GetCurrentPid()) { //the lock is not grabbed properlly
	printf("CondBroadcast: Process %d did not grab the lock for condition variable %d\n", GetCurrentPid(), (int)(condition-conds));
  	RestoreIntrs (intrs);
	return SYNC_FAIL;
  }
  else { 
    while (!AQueueEmpty(&condition->waiting)) { // there is a process to waiting for the wake up.The only different between CondBroadcast and CondSignal is that broadcast uses the while loop, which should allow multiple callees to get this condition.
      l = AQueueFirst(&condition->waiting);
      pcb = (PCB *)AQueueObject(l);
      if (AQueueRemove(&l) != QUEUE_SUCCESS) { 
        printf("FATAL ERROR: could not remove link from the condition variable queue in CondSignal!\n");

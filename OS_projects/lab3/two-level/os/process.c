...
extern l2_pagetable l2[MEM_L2TABLE_SIZE];
  	...
  	if(pcb->pagetable[i]!=NULL){
			...
			*(addr-4)=0;
			for(j=0;j<MEM_L2TABLE_SIZE;j++){
				if((addr[j]&MEM_PTE_VALID)>0){
					addr[j] = addr[j]^MEM_PTE_VALID;
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  pcb->sysStackArea = pcb->sysStackArea - 1;
	...
	for (i = 0; i < 4; i++){
     ...
     if ((address[i] = MemoryAllocPage()) == MEM_FAIL){
	  	exitsim();
		}else{
	...
	if ((address[255] = MemoryAllocPage()) == MEM_FAIL){
  	exitsim();
  }
  else{
  ...
  if (isUser) {
    ...
    stackframe[PROCESS_STACK_USER_STACKPOINTER] = ((1<<22) - 1) - (0x3);

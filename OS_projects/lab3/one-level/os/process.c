...
void ProcessFreeResources (PCB *pcb) {
  ...
  //free page table
  for(j=0;j<MEM_L1TABLE_SIZE;j++){
  	// check valid bit and free the page
  	if((pcb->pagetable[j]&0x1)==1){
  		MemoryFreePage(pcb->pagetable[j]>>MEM_L1FIELD_FIRST_BITNUM);
  		pcb->pagetable[j]=0;
  	}
  }
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  p_page_num = MemoryAllocPage();
  pcb->sysStackArea = MEM_PAGESIZE*p_page_num;
  stackframe = (uint32*)((MEM_PAGESIZE*(p_page_num+1)-1)&(~0x3));

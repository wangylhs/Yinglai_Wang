...
/**
 * Calls methods to draw the game display and handles mouse events within the
 * game display
 * 
...
public class GameCanvas implements MouseListener, MouseMotionListener {
	...
	/**
	 * Provides the game level, score and lives for display. Also provides
	 ...
	 *            Array of projectiles
	 **/
	public GameCanvas(GameSounds inGameSounds, Mushroom inMushrooms[][],
		...
		// Setup the canvas. Canvas is drawn with two extra spaces at the bottom
		// for game status display
		c = new Canvas();
		c.setSize(Settings.width * Settings.scale, (Settings.height + 2)
				* Settings.scale);
		c.setVisible(true);
	...
	public void drawFrame() {
		...
		try {
			...
			// Canvas is drawn with two extra spaces at the bottom for game
			// status display
			g.fillRect(0, 0, Settings.width * Settings.scale,
					(Settings.height + 2) * Settings.scale);
	...
	/**
	 * If the mouse button is pressed and the game is unpaused, try to fire the
	 * projectile
	 * 
	 * @param e
	 *            MouseEvent that occured
		...
		if (!myGameLogic.paused) {
			...
			if (myGameLogic.gameOverFlag) {
				...
				if (myShip.invulnerableTime == 0)
					myShip.fire();
				// Otherwise, turn off automatic ship firing and try to fire a
				// single projectile
			} else {
				myShip.firing = false;
				if (myShip.invulnerableTime == 0)
	...
	/**
	 * If the mouse button is released and the game is unpaused, turn off
	 * automatic firing
	 * 
	 * @param e
	 *            MouseEvent that occured
	...
	/**
	 * If the mouse is moved, the game is unpaused and not over, then try to
	 * move the ship
	 * 
	 * @param e
	 *            MouseEvent that occured
	...
	public void mouseMoved(MouseEvent e) {
		...
		if ((!myGameLogic.paused) && (!myGameLogic.gameOverFlag)) {
			...
			// Test for movement range bounds. If beyond bounds, change cursor
			// to a cursor. If in bounds, change cursor to invisible.
			if (tryX < 0) {
				myShip.tryLoc.x = 0;
			...
			if (tryY < (Settings.height - Settings.shipVerticalRange - 1)
					* Settings.scale) {
				myShip.tryLoc.y = (Settings.height - Settings.shipVerticalRange - 1)
						* Settings.scale;
				nextCursor = Cursor.getDefaultCursor();
	...
	/**
	 * Same as mouseMoved. Lets the user move the ship while holding the mouse.
	 * Useful for super laser.
	 * 
	 * @param e
	 *            MouseEvent that occured
	 **/

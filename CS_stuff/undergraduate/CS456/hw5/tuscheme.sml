(* this problem is done by team, Yinglai Wang and Yizhen Wei*)
	...
	(case ty(f) of
		 CONAPP (TYCON "function", [CONAPP (TYCON "tuple", args), result]) => 
			if eqTypes(args,(map ty a)) then
				...
				raise TypeError("argtypes don't match")
...
fun elabdef (d, gamma, delta)  =
	...
	VAL (n, e) => let val tau=typeof(e,gamma,delta)
	 ...
	 | VALREC (n,t,e) =>
           let val gamma' = bind (n, t, gamma)
               val _ = typeof (e, gamma', delta)
           in
               (gamma', typeString t)
           end
               (* if eqType(t,typeof(e,gamma,delta)) then *)

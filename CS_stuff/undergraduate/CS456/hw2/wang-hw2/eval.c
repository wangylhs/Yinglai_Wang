...
/* eval helpers 40a */
/* eval have been modified for hw2 */
...
/* eval.c 36a */
/* local variable environment been added to all cases of eval */
Value eval(Exp e, Valenv globals, Funenv functions, Valenv formals, 
    ...
    case LITERAL:
        ...
        return e->u.literal;
    case VAR:
        /* global are hided, and local as the base case */
    ...
    case APPLY:
        /* evaluate [[e->u.apply]] and return the result 39b */
        {
            ...
            switch (f.alt) {
            case USERDEF:
                /* USERDEF have been modified to parse local variables environment */

...
/* eval helpers 40a */
static Valuelist evallist(Explist el, Valenv globals, Funenv functions, Valenv formals, Valenv locals) {
    if (el == NULL) {
        return NULL;
    } else {
...
Value eval(Exp e, Valenv globals, Funenv functions, Valenv formals, Valenv locals) {
        ...
        /* evaluate [[e->u.apply]] and return the result 39b */
    {
            ...
            /* apply [[f.u.userdef]] and return the result 40b */
        {
            ...
            Namelist ll = f.u.userdef.locals;
            int length = lengthNL(ll);
            Valuelist vlzero;
            int c;
            for(c = 0; c<length; c++){
                vlzero = mkVL(0, vlzero);
            }
            
            checkargc(e, lengthNL(nl), lengthVL(vl));
            return eval(f.u.userdef.body, globals, functions, mkValenv(
                            nl, vl), mkValenv(ll, vlzero));
        }
        case PRIMITIVE:
